...
class BalancedImageDatasetFromDirectoryTest(keras_parameterized.TestCase):
    ...
    def _prepare_directory(
        ...
        temp_dir = os.path.join(self.get_temp_dir(), str(np.random.randint(1000000.0)))
        os.mkdir(temp_dir)
    ...
    def test_balanced_image_dataset_from_directory_errors(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "`samples_per_epoch` must be divisible by batch_size"
        ):
            _ = balanced_image_dataset.balanced_image_dataset_from_directory(
                directory,
                num_classes_per_batch=4,
        ...
        with self.assertRaisesRegex(
            ValueError, "You can only pass `samples_per_epoch` if safe_triplet is True"
        ):
            _ = balanced_image_dataset.balanced_image_dataset_from_directory(
                directory,
                num_classes_per_batch=4,
